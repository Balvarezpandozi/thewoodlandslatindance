...
describe('GET /', () => {
    ...
    it('responds with an HTML page', async () => {
        const barbersMock = jest.spyOn(Barbers, 'find');
        barbersMock.mockImplementation(() => {return []});
        const scheduleMock = jest.spyOn(Schedule, 'findOne');
        scheduleMock.mockImplementation(() => {return {week:[]}});
        const productsMock = jest.spyOn(Products, 'find');
        productsMock.mockImplementation(() => {return []});
